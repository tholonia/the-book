...
def mdot_rrg(v, c):
    sphere(pos=vector(v[0], v[1], v[2]), radius=.1, color=c, texture="map_rrg.png")

def mdot(v, c):
    sphere(pos=vector(v[0], v[1], v[2]), radius=.05, color=c,opacity=1)
def mdot0(v, c):
...
def itet(hyp, gen):
    ...
    curve(pos=[p_red_dot, p_white], color=WHITE)

    p_lightblue_dot = calcChildPt(p_blue_dot, p_white)
    mdot(p_lightblue_dot, LIGHTBLUE)
    curve(pos=[p_blue_dot, p_white], color=WHITE)

    p_lightgreen_dot = calcChildPt(p_green_dot, p_white)
    mdot(p_lightgreen_dot, WHITE)
    curve(pos=[p_green_dot, p_white], color=WHITE)
